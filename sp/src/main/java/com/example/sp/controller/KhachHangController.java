...
public class KhachHangController {
    ...
    private byte[] exportToExcel(List<KhachHang> danhSach) throws IOException {
        ...
        try (Workbook workbook = new XSSFWorkbook();
            ...
            String[] headers = {"STT", "Ma KH", "Ten khach hang", "Email", "SDT", "Dia chi", "Gioi tinh", "Ngay sinh", "Trang thai"};
            ...
            for (KhachHang kh : danhSach) {
                ...
                row.createCell(5).setCellValue(value(kh.getDiaChi()));
                row.createCell(6).setCellValue(value(kh.getGioiTinh()));
                row.createCell(7).setCellValue(kh.getNgaySinh() == null ? "" : kh.getNgaySinh().format(dateFormatter));
                row.createCell(8).setCellValue(Boolean.TRUE.equals(kh.getTrangThai()) ? "Hoat dong" : "Ngung hoat dong");
