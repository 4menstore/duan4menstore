...
public class SanPhamService {
    ...
    private void validatePrices(BigDecimal giaNhap, BigDecimal giaBan) {
        if (giaNhap != null && giaNhap.compareTo(BigDecimal.ZERO) < 0) {
            throw new RuntimeException("Giá nhập không được nhỏ hơn 0");
        }
    ...
    private void applyVariantFields(ChiTietSanPham ct, ChiTietSanPhamRequest ctDto, String nguoiThucHien, boolean creating) {
        ...
        validatePrices(ctDto.getGiaNhap(), ctDto.getGiaBan());
    ...
    public ChiTietSanPham updateVariant(Integer idSpct, ChiTietSanPhamUpdateRequest request) {
        ...
        BigDecimal giaNhap = request.getGiaNhap() != null ? request.getGiaNhap() : ct.getGiaNhap();
        BigDecimal giaBan = request.getGiaBan() != null ? request.getGiaBan() : ct.getGiaBan();
        validatePrices(giaNhap, giaBan);
        ...
        if (request.getGiaNhap() != null) ct.setGiaNhap(request.getGiaNhap());
        if (request.getGiaBan() != null) ct.setGiaBan(request.getGiaBan());
